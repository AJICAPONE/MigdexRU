$(document).ready(function () {
    ...
    $('.header-fixed-menu--hidden').click(function () {
        $(this).children('.header-fixed--list__wrap').slideToggle(200);
    });
    $(document).click(function (e) {
        if (!$(e.target).closest(".header-fixed--list__wrap,.header-fixed-menu--hidden").length) {
            $('.header-fixed--list__wrap').slideUp(200);
        }
        e.stopPropagation();
    });
    ...
    $('.migdex-lk-content--product--pay--item').click(function () {
       $(this).addClass('checked').siblings().removeClass('checked');
    });

    $('.answer-on-question--top').click(function () {
        $('.answer-on-question--bottom').slideUp(200);
    });
    ...
    function countClient(){
        marker = false;
        $('.info-about-us--title').each(function () {
            var $this = $(this);
            var currentNumber = $this.text();
            var dataNumber = $this.attr('data-count-too');
            var dataAttr = $this.attr('data-count-attr');
            $({numberValue: currentNumber}).animate({numberValue: dataNumber}, {
                duration: 2000,
                delay: 1000,
                easing: 'linear',
                progress: function() {
                    $this.text(Math.ceil(this.numberValue) + dataAttr);
                }
            });
        });
    ...
    $(window).on('scroll',function(){
        var wt = $(window).scrollTop(); // Пройденая высота
        var wh = $(window).height(); // Высота окна
        var eh = $('.info-about-us').outerHeight();
        var dh = $(document).height();
        var contentNav = $('.info-about-us'); // Высота до блока
        if (contentNav.length) {
            var et = contentNav.offset().top;
